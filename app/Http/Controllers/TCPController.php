...
class TCPController extends Controller
{
    ...
    public function __construct($bind_ip,$port)
    {
        ...
        TCPController::debug("Listenting for connections on {$bind_ip}:{$port}");
    }
    ...
    */
    public function loop_once()
    {
        ...
        if($select < 1)
        {
            //    TCPController::debug("Problem blocking socket_select?");
            return true;
        }
        ...
        if(in_array($this->master_socket, $read))
        {
            for($i = 0; $i < $this->max_clients; $i++)
            {
                if(empty($this->clients[$i]))
                {
                    ...
                    $this->clients[$i] = new TCPControllerClient($this->master_socket,$i);
                    $this->trigger_hooks("CONNECT",$this->clients[$i],"");
                    break;
                }
                elseif($i == ($this->max_clients-1))
                {
                    TCPController::debug("Too many clients... :( ");
                }
            }
        ...
        for($i = 0; $i < $this->max_clients; $i++) // for each client
        {
            if(isset($this->clients[$i]))
            {
                if(in_array($this->clients[$i]->socket, $read))
                {
                    ...
                    {
                        TCPController::debug("{$i}@{$this->clients[$i]->ip} --> {$input}");
    ...
    */
    ...
    {
        $i = $client_index;
        TCPController::debug("Client {$i} from {$this->clients[$i]->ip} Disconnecting");
    ...
    */
    public function trigger_hooks($command,&$client,$input)
    {
        ...
        {
            $call = new ServerController();
            foreach($this->hooks[$command] as $function)
            {
                TCPController::debug("Triggering Hook '{$function}' for '{$command}'");
                $continue = call_user_func(array($call,$function),$this,$client,$input);
...
/*!    @class        TCPControllerClient
    @author        Navarr Barnier
    @abstract    A Client Instance for use with TCPController
 */
class TCPControllerClient
{
    /*!    @var        socket
    ...
    /*!    @var        server_clients_index
        @abstract    int - The index of this client in the TCPController's client array.
     */
    ...
    public function __construct(&$socket,$i)
    {
        ...
        TCPController::debug("New Client Connected");
